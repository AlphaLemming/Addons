		...
		if et ~= EQUIP_TYPE_INVALID then
			...
			for set = 1,MAXSLOT do
				for _,x  in pairs(DUPSLOTS) do
					if(AG.setdata[set].Gear[x].id == DRAGINFO.uid) then DRAGINFO.slot = {x}; break end
				end
			end
...
function AG.LoadItem(nr,slot,set)
	...
	if AG.setdata[nr].Gear[slot].id ~= 0 then
		if Id64ToString(GetItemUniqueId(BAG_WORN,SLOTS[slot][1])) ~= AG.setdata[nr].Gear[slot].id then
			local bagSlot = AG.GetItemFromBag(AG.setdata[nr].Gear[slot].id)
...
function AG.UpdateEditPanel(nr)
	...
	if set.gear > 0 and AG.setdata[set.gear].Gear[1].id ~= 0 then val = Zero(set.icon[1]) or AG.GetSetIcon(set.gear,1) else val = 'x.dds' end
	AG_PanelSetPanelScrollChildEditPanelBar1IconTex:SetTexture(val)
	if set.gear > 0 and AG.setdata[set.gear].Gear[3].id ~= 0 then val = Zero(set.icon[2]) or AG.GetSetIcon(set.gear,2) else val = 'x.dds' end
...
function AG.SetOptions()
	...
	if OPT[5] then
		...
		EM:UnregisterForEvent('AG_Event_Charge',EVENT_INVENTORY_SINGLE_SLOT_UPDATE)
		Hide(AG_Charge1)
		Hide(AG_Charge2)
	end
...
function AG.TooltipShow(c,id)
	if OPT[8] and id and c then
		local sets = ''
		for nr = 1,MAXSLOT do
			if AG.setdata[nr].Set.gear > 0 then
				for slot = 1,14 do
					if AG.setdata[AG.setdata[nr].Set.gear].Gear[slot].id == id then
						if sets == '' then sets = sets..nr else sets = sets..', '..nr end
					end
				end
			end
		end
		if sets ~= '' then c:AddLine(ZOSF(L.SetPart,sets),'AGFontSmall') end
	end
end
